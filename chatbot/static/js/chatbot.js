...
function send(text, isSpeech = false) {
  ...
  $.ajax({
    ...
    dataType: "json",
    data: JSON.stringify({ student_id: student_id, query: text }),
    success: function (data) {
      console.log(data);

      // Pass the response into the method 
      newRecievedMessage(data, isSpeech);
...
function newRecievedMessage(messageText, isSpeechEnabled) {
  voiceMessage = ""
  if (Object.keys(messageText).length != 0) {
    if (messageText["recommended_modules"] !== undefined) {
      voiceMessage += `Here are some recommendation based on your recent query `
      formattedMessage = `<p>Here are some recommendation based on your recent query: </p><ul class="unordered-list">`;
      messageText["recommended_modules"].forEach(element => {
        formattedMessage += `<li><a target="_blank" href = "${element['module_link']}">${element['module_name']}</a></li>`
        voiceMessage += element['module_link']
      })
      formattedMessage += "</ul>"
      messageText = formattedMessage;
      voiceMessage += ` please click the below links to go to module`
      if(isSpeechEnabled){
        speechResponse(voiceMessage)
      }
    }
    else if (messageText["recommended_course"] !== undefined) {
      messageVoice = `We couldn't find any recommendation in your subscribed courses. Here are some courses which you can subscribe to get answers ` 
      formattedMessage = `<p>We couldn't find any recommendation in your subscribed courses. Here are some courses which you can subscribe to get answers: </p><ul class="unordered-list">`;
      messageText["recommended_course"].forEach(element => {
        formattedMessage += `<li><a target="_blank" href = "${element['course_landing_page']}">${element['course_name']}</a></li>`
        messageVoice += element['course_landing_page'] + ' '
      })
      formattedMessage += "</ul>"
      messageText = formattedMessage;
      messageVoice += `please click the below links to go to module`
      if(isSpeechEnabled){
        speechResponse(messageVoice)
      }
    }
    else {
      messageText = "We haven't found what you looking for, Please try again with some other query."
      if(isSpeechEnabled){
        speechResponse(messageText)
      }
    }
  }
